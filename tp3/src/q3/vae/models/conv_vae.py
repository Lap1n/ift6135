...
class ConvVAE(nn.Module):
    ...
    def fit(self, trainloader, validloader, lr=0.001, batch_size=128,
            num_epochs=10):
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, (inputs, _) in enumerate(validloader):
                ...
                if batch_idx == 0:
                    ...
                    comparison = torch.cat([inputs.view(-1, self.nChannels,
                                                        self.width,
                                                        self.height)[:n],
                                            outputs.view(-1, self.nChannels,
                                                         self.width,
                                                         self.height)[:n]])
                    save_image(comparison.data.cpu(),
                               'results/vae/reconstruct/reconstruction_' + str(
                                   epoch) + '.png', nrow=n)
            ...
            save_image(
                sample.data.view(64, self.nChannels, self.width, self.height),
