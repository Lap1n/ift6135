...
from q3 import utils
from q3.model import Generator, Discriminator
...
parser.add_argument("--lambda_grad_penality", type=float, default=10,
                    help="WD gradient penality")
...
def compute_gradient_penality(D, x_real, x_fake):
    ...
    grads = autograd.grad(outputs=D_z,
                          inputs=z,
                          grad_outputs=torch.ones(D_z.size(),
                                                  device=get_device()),
                          create_graph=True,
                          retain_graph=True,
                          only_inputs=True, )[0]
...
for epoch in range(args.n_epochs):
    for i, (x_real, y_real) in enumerate(train):
        ...
        gradient_penality = args.lambda_grad_penality * compute_gradient_penality(
        ...
        if i % args.log_interval == 0:
            csv_logger.write(epoch, i, wd.item(), gradient_penality.item(),
                             wd_loss.item(), generator_loss.item())
            print(
                f"Epoch={epoch}, i={i}, wd={wd_loss}, g_loss={generator_loss}")
            utils.log(epoch, i, wd_loss, generator_loss)
    utils.save_generator_and_discriminator(experiment_path, g=generator,
                                           d=discriminator)
