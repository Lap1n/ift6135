...
def compute_gradient_penality(T, x, y, lambda_grad_penality):
    ...
    z = a * x + (1 - a) * y
    T_z = T(z)
    grads = autograd.grad(outputs=T_z, inputs=a, grad_outputs=torch.ones(T_z.size()),
...
def WassersteinDistance(p, q, n_mini_batch=1000, lambda_grad_penality=10):
    ...
    optimizer = optim.SGD(T.parameters(), lr=1e-3)
...
if __name__ == "__main__":
    ...
    q = iter(distribution1(1, batch_size))
    D, jsd = JSD(p, q, n_mini_batch=n_mini_batch)
    T, wd = WD(p, q, n_mini_batch=n_mini_batch, lambda_grad_penality=0)
    print(jsd, wd)
    ##
    batch_size = 512
    n_mini_batch = 100
